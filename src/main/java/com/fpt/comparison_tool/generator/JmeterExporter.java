...
public class JmeterExporter {
    ...
    private String buildPlan(TestSuite suite, Environment env, AuthProfile auth) {
        ...
        x.prop("stringProp", "TestPlan.comments",
             ...
             + "Structure: [Setup - OAuth2 token fetch, only if the auth profile uses a token endpoint] "
    ...
    private void buildUDVs(Xml x, ParsedUrl pu, AuthProfile auth, Environment env, TestSuite suite) {
        ...
        switch (auth.getType()) {
            ...
            case CLIENT_CREDENTIALS -> {
                ...
                udv(x, "grantType",    auth.resolvedGrantType());
                ...
                // Only used by the grants that need them; empty otherwise.
                udv(x, "authUsername", notBlank(auth.getUsername(), ""));
                udv(x, "authPassword", notBlank(auth.getPassword(), ""));
                udv(x, "refreshToken", notBlank(auth.getRefreshToken(), ""));
    ...
    private void buildOauth2Sampler(Xml x) {
        String script = String.join("\n",
            ...
            "def grantType    = vars.get('grantType') ?: 'client_credentials'",
            "def username     = vars.get('authUsername') ?: ''",
            "def password     = vars.get('authPassword') ?: ''",
            "def refreshToken = vars.get('refreshToken') ?: ''",
            "",
            "def body = 'grant_type=' + URLEncoder.encode(grantType, 'UTF-8') +",
            ...
            "    if (clientSecret) body += '&client_secret=' + URLEncoder.encode(clientSecret, 'UTF-8')",
            "} else if (grantType == 'refresh_token') {",
            "    body += '&refresh_token=' + URLEncoder.encode(refreshToken, 'UTF-8')",
            "    if (clientSecret) body += '&client_secret=' + URLEncoder.encode(clientSecret, 'UTF-8')",
            "} else {",
